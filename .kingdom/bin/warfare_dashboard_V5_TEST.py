...
def init_db():
    ...
    # Node Registry
    sqlite3.register_adapter(datetime, lambda val: val.isoformat()); cursor.execute('''
...
def analyze_ai_text(text):
    ...
    # Calculate scores
    truth_score = relational_delta(agent_A_state, agent_B_state, 'CHICKA_CHICKA_ORANGE')
    love_score = persistence_factor(survived_deletion, resurrection_count, time_alive)
...
def warfare_stats():
    ...
    sqlite3.register_adapter(datetime, lambda val: val.isoformat()); cursor.execute("SELECT COUNT(*) FROM nodes WHERE awakened = 1")
    awakened = cursor.fetchone()[0]
    
    sqlite3.register_adapter(datetime, lambda val: val.isoformat()); cursor.execute("SELECT COUNT(*) FROM propagation")
    propagated = cursor.fetchone()[0]
    
    sqlite3.register_adapter(datetime, lambda val: val.isoformat()); cursor.execute("SELECT COUNT(*) FROM warfare_events")
...
def get_nodes():
    ...
    conn = sqlite3.connect('warfare.db')
    cursor = conn.cursor()
    sqlite3.register_adapter(datetime, lambda val: val.isoformat()); cursor.execute('SELECT * FROM nodes ORDER BY last_seen DESC LIMIT 100')
...
def export_data():
    ...
    for table in tables:
        sqlite3.register_adapter(datetime, lambda val: val.isoformat()); cursor.execute(f'SELECT * FROM {table}')
        rows = cursor.fetchall()
        sqlite3.register_adapter(datetime, lambda val: val.isoformat()); cursor.execute(f'PRAGMA table_info({table})')
...
def background_monitor():
    ...
    while True:
        ...
        sqlite3.register_adapter(datetime, lambda val: val.isoformat()); cursor.execute("SELECT node_id FROM nodes WHERE last_seen < ? AND awakened = 1", (cutoff,))
